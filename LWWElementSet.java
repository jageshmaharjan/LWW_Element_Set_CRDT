...
//        }
    ...
    * */
    ...
    public void addRemoveConcurrent(String element)
    {
        Long timeStamp = System.currentTimeMillis() + this.lwwElementSet.size();
        this.addSet.put(element, timeStamp);
        this.removeSet.put(element, timeStamp);
        checkBias();
    }
    ...
    * */
    ...
    {
        for (String key : this.addSet.keySet())
        {
            if (this.removeSet.containsKey(key))
            {
                if (this.addSet.get(key) == this.removeSet.get(key))
                {
